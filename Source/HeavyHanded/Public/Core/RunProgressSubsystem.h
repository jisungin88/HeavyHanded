...
public:
	...
	/**
	 * 관전으로 형기를 마친 사람들을 체포 명단에서 뺀다. (서버 전용 — 작업 레벨의 결과 확정 시)
	 *
	 * 기획서 2장의 체포 대가는 "다음 작업 관전" 이다. 한 판을 관전으로 보냈으면 그것으로
	 * 값을 치른 것이므로 비용 없이 풀린다 — 그래서 TryRescue 와 별개의 함수다.
	 * TryRescue 는 **형기를 건너뛰는** 값이고, 이쪽은 형기를 다 산 결과다.
	 *
	 * 잡혀 있지 않은 사람이 섞여 있어도 그냥 무시한다. 호출부는 "이 판의 관전자 전원" 을
	 * 넘기는데, 그중에 다른 이유로 관전 중인 사람(추후 난입 대기 등)이 있을 수 있다.
	 */
	...
	//   이 클래스가 답하지 않는다. 사실만 들고 있고, 그 이동은 그것을 읽는 쪽(세션 파트의
	//   ServerTravel)이 정한다. 최종 성공 처리가 아직 기획 미확정이라 더더욱 떠안지 않는다.
	//
	//   **아래 '출발' 과 혼동하지 말 것.** 은신처 → 작업 장소는 고른 장소가 곧 목적지라
	//   판단할 것이 없어서 여기가 한다. 작업 장소 → 그다음은 판정이고, 그 판정이 미확정이다.
	...
	// ── 출발 ──
	//
	// 기획서 2장 — 은신처에서 다음 목표를 고르고 떠난다. 여기가 그 '떠난다' 다.
	//
	// [왜 은신처 GameMode 가 아니라 여기인가]
	//   떠날 때 들고 가야 하는 것(고른 진입점 · 역할 · 구매 장비 · 팀 골드)이 전부 여기 있고,
	//   ServerTravel 을 견디는 것도 여기뿐이다. 은신처 쪽은 이 함수를 부르기만 하면 된다 —
	//   그래서 세션 · UI 파트가 건드릴 코드가 호출 한 줄로 끝난다.

	/**
	 * 이 장소로 출발한다. 전원을 데리고 작업 레벨로 ServerTravel 한다. (서버 전용)
	 *
	 * @param SiteTag  갈 장소(Site.*). UHeistSettings::SiteLevels 에 등록돼 있어야 한다
	 * @return 떠났으면 true. 아래 경우에는 false 이고 **아무 일도 일어나지 않는다**
	 *           - 서버가 아니다
	 *           - 무효 태그이거나 SiteLevels 에 없는 장소다 (엉뚱한 맵을 열지 않는다)
	 *
	 * [진입점을 여기서 검사하지 않는다] 안 골랐어도 떠날 수 있다.
	 *   레벨의 기본 진입점으로 떨어지는 것이 정상 동작이고, 그 판정은 도착한 뒤
	 *   HeistEntryGate 가 한다 — 여기서 막으면 PIE 로 레벨을 여는 경로까지 같이 막힌다.
	 *
	 * [BeginNewRun 을 부르지 않는다] 출발과 "새 판 시작" 은 다른 사건이다.
	 *   재도전으로 같은 장소에 다시 들어가는 경우가 있고, 그때 명단과 구매 장비를 비우면
	 *   산 장비가 사라진다 (기획서 2장 — 재도전 시 구매 장비는 유지).
	 *
	 * [떠난 뒤에는 돌아오지 않는다] 비-심리스 ServerTravel 이라 이 호출 뒤의 코드는
	 *   같은 월드에서 이어지지 않는다. 출발 직전에 할 일이 있으면 이 함수를 부르기 전에 할 것.
	 *
	 * [BP 에서 부를 때는 서버에서 도는 그래프여야 한다]
	 *   `GM_ShelterGameMode` 처럼 서버에만 있는 BP 가 부르면 된다. 위젯 그래프에서 바로 부르면
	 *   클라이언트에서 도는 것이라 아무 일도 일어나지 않고 로그만 남는다 —
	 *   버튼은 PlayerController 의 Server RPC 를 거쳐 여기로 와야 한다 (문서 02 4장).
	 *
	 *   판정은 전부 이 함수 안에서 끝난다. BP 는 부르기만 하고 결과로 분기하지 않는다 (문서 03 4장).
	 */
